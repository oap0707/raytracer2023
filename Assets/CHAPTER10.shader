...
Shader "Unlit/SingleColor"
{
	...
	class Camera {
		...
		void makeCamera(float vfov, float aspect) {
			theta = vfov * 3.14159265358979 / 180.0;
			halfHeight = tan(theta * 0.5);
	...
	bool hitSphere (Sphere sph, ray r, float t_min, float t_max, out hit_record rec) {
		...
		if (discriminant > 0.0) {
			...
			if (temp < t_max && temp > t_min) {
				...
				//if (sph.radius < 0) {
					//rec.makeN((sph.center - rec.p) / sph.radius);
				//}
	...
	float3 hash31(const float p)
{
	...
	void getsphere(int i, out Sphere sph) {
		...
		float R = cos(3.14159265358979/4);
		...
		//if (i == 0) {
			...
			//sph.make(vec3(0.0, 0.0, -1.0), 0.5, mat); // last element is diffuse or metal
		//}
		//if (i == 1) {
		...
		//if (i == 3) {
			...
			//material mat;
			//mat.make(2, vec3(0.0,0.0,0.0), 0, 1.5);
			//sph.make(vec3(-1.0, 0.0, -1.0), 0.5, mat);
		//}
		//if (i == 4) {
	...
	bool scatter(ray rIn, hit_record rec, float2 random, out vec3 attenuation, out ray rScattered) {
		...
		if(rec.mat.type == 2) {
			...
			if (hash13(random_unit_sphere(random)) < reflectProb) {
				rScattered.make(rec.p, reflected);
			}
			else {
...
////////////////////////////////////////////////////////////////////////////////////////////////////////
	fixed4 frag(v2f i) : SV_Target
	{
		...
		Camera cam;
		cam.makeCamera(90, float(nx)/float(ny));
